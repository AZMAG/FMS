$(async function() {
    ...
    async function doubleHighlight(det_num) {
        ...
        const res = await detectorsLyrView.queryFeatures({ where: 'det_num = ' + det_num, returnGeometry: true });
    ...
    async function unhighlight(unhighlight, currentlyHighlighted) {
        ...
        let detLayer = map.findLayerById("detectors");
        let oids = await detLayer.queryObjectIds({ where: `det_num in(${newHighlightDetNums.join(', ')})` });
    ...
    function setupSelectPolygonTool() {
        return new Promise(
            function(res, rej) {
                ...
                require(["esri/widgets/Sketch/SketchViewModel", "esri/layers/GraphicsLayer"], function(SketchViewModel, GraphicsLayer) {
