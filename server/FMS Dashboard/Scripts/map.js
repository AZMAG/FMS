...
async function _setupMap(params) {
  ...
  if (options.detectorLayer) {
    ...
    if (options.detectorLayer.visible === false) {
      detLayer.visible = false;
    }
    if (options.detectorLayer.definitionExpression) {
      detLayer.definitionExpression =
        options.detectorLayer.definitionExpression;
    }
    map.add(detLayer);
...
async function getJsonByUrl(url) {
    const res = await fetch(`${window.location.origin}${appPath}${url}`);
