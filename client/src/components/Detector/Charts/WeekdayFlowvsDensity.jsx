...
export default function WeekdayFlowvsDensity({ det_num, reportId, year }) {
    ...
    useEffect(() => {
        (async () => {
            ...
            if (reportId) {
                res = await axios.get(
                    apiUrl + "Detector/FlowVsDensityByReportId",
                    {
                        params: {
                            reportId,
                        },
                ...
                res = await axios.get(
                    apiUrl + "Detector/FlowVsDensityByDetNum",
                    {
                        params: {
                            det_num,
                            year,
            ...
            {series.length ? (
                <div id="flow-vs-density">
                    <ScatterChart
                        ...
                        xLabel="Flow (Veh/hr)"
                        yLabel="Occupancy %"
                        title="Flow vs Density - all data rows & 5-min weekday average"
                    />
                </div>
            ) : (
                <LoadingChart />
