  ...
  React.useEffect(() => {
    (async () => {
      if (store.corridorMap.corridorLayer) {
        const res = await store.corridorMap.corridorLayer.queryFeatures();
        ...
        res.features.forEach((feature) => {
          const validity =
            feature.attributes["Validity" + store.corridorMap.selectedYear];
  ...
  return (
    <div className="h-52">
      <p className="px-3 pb-1 text-lg">
        <span className="font-bold">{numCorridors}</span>&nbsp;corridors
        currently shown.
      </p>
            ...
            return (
              <div key={i} className="flex items-center">
                ...
                <span className="flex w-full" key={i}>
                  <span className="w-12 text-center">{cat.count}</span>
                  <span>{cat.name}</span>
                </span>
              </div>
            );
